...
export default function Quiz({ quizId }: { quizId: string }) {
  ...
  if (!quiz) {
    return (
      <div className="p-4 border border-edge rounded-lg bg-amber-50 dark:bg-amber-950/30 text-amber-800 dark:text-amber-300 text-sm flex items-center gap-2">
  ...
  if (showResult) {
    ...
    return (
      <div className="border border-edge rounded-lg p-6 text-center space-y-4 animate-scale-in">
        ...
        <button
          onClick={handleRetry}
          className="inline-flex items-center gap-2 px-4 py-2 bg-accent/10 text-accent rounded-lg text-sm hover:bg-accent/20 transition-colors"
        >
  ...
  return (
    <div className="border border-edge rounded-lg p-5 space-y-4 animate-slide-up">
      <div className="flex items-center justify-between text-sm">
        <span className="font-bold uppercase tracking-widest text-accent">{quiz.title}</span>
        <span className="text-muted">{current + 1} / {total}</span>
      </div>
        ...
        {Array.from({ length: total }).map((_, i) => (
          <div
            ...
            className={`h-1 flex-1 rounded-full transition-colors ${
              i === current ? "bg-accent" : revealed.has(i) ? "bg-accent/30" : "bg-surface-alt"
            }`}
          />
        ))}
      </div>
      ...
      <div className="space-y-2">
        {question.options.map((opt, idx) => {
          let btnClass = "border border-edge bg-surface hover:bg-surface-alt";
          if (isRevealed) {
            if (idx === question.correctIndex) {
              ...
              btnClass = "border-edge opacity-50";
            }
          } else if (idx === selectedAnswer) {
          ...
          return (
            ...
            >
              <span className="w-6 h-6 rounded-full border border-edge flex items-center justify-center text-xs shrink-0 font-mono">
      ...
      <div className="flex justify-end">
        {!isRevealed ? (
          <button
            ...
            className="px-4 py-2 bg-accent/10 text-accent rounded-lg text-sm hover:bg-accent/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            确认
          </button>
        ) : (
