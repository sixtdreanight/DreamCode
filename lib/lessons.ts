...
打开 AI 工具，输入：
> 你好！我想学编程，但我完全零基础。请用最简单的话告诉我，编程到底是什么？

看看 AI 怎么回答。

> 常见误区
> "我英语不好学不了编程"——AI 工具都支持中文。
> "编程需要数学好"——做网页不需要高深数学，逻辑思维比数学重要。

### 你学会了
- 编程就是告诉电脑帮你做事
- Vibe Coding = 你描述想法，AI 写代码
- 你能做：网站、游戏、工具、App
`,
    hasChat: true,
    hasPlayground: false,
...
> 常见误区
> "AI 替我搞定一切"——AI 是助手不是神。你需要判断它给的答案对不对。把 AI 当实习生：能力强，需要你的指导。
...
用你自己的话向一个朋友解释什么是 Vibe Coding。如果他听懂了，说明你真的理解了。

> 常见误区
> "我描述得很清楚 AI 还是不对，我不适合学编程"——不是你的问题。换个说法重试，分步骤慢慢来。核心是反复迭代，不是一次成功。

### 你学会了
- Vibe Coding = 想-说-看-改-重复
- 核心能力：描述 + 判断 + 迭代
- 不需要手写代码，但需要清楚表达想法
`,
    hasChat: true,
...
用描述公式写一条完整 Prompt（做什么 + 给谁用 + 功能 + 风格），发给 AI 看看生成的代码质量。

> 常见误区
> "Prompt 很长但还是不对"——换个角度。不说"按钮要好看"，说"圆角矩形，蓝色背景，白色文字，hover 变深"。
...
找一个你喜欢的网站页面，用文字描述它的布局和风格给 AI，让它复刻一个类似的。

> 常见误区
> "参考就是抄袭"——参考不是抄袭。参考是"我喜欢这个布局"，抄袭是复制粘贴。学习阶段参考别人是很好的方式。
...
把你手头的项目拆成 5 个小步骤。然后依次发给 AI，看看分步生成的效果是不是更好。

> 常见误区
> "一次生成越多越好"——代码越多越难检查。10 行能看懂，100 行就蒙了。每步只加一个功能。
...
打开右侧的 Prompt Playground，选一个题目开始做。不满意就修改描述重试。

> 常见误区
> "我看不懂生成的代码"——刚开始看不懂完全正常。目标是"做出东西"，不是"看懂每一行"。做多了自然懂。

### 你学会了
- 在 Playground 描述需求，AI 生成代码
- 代码复制到 .html 文件就能打开
- 不满意就修改描述重试
`,
    hasChat: false,
...
把上节课的代码拿出来，告诉 AI 做 3 个修改：颜色、布局、加一个新功能。每次只改一样。

> 常见误区
> "越改越差"——保存每个版本（v1.html, v2.html）。改坏了就把旧代码贴回去说"还原到之前的版本"。

### 你学会了
- 核心是快速迭代：生成-看效果-提修改-再生成
- 每次只改一样：样式/功能/结构
- 保存每个版本，改坏了可回退
`,
    hasChat: false,
...
把做过的所有项目放到一个文件夹 my-projects 里。给每个文件取有意义的名字。

> 常见误区
> "文件乱七八糟没关系"——项目多了找不到文件更崩溃。从现在开始每个项目一个文件夹。

### 你学会了
- 代码保存到本地 .html 文件
- 命名有意义，一个项目一个文件夹
- CodePen 快速测试，GitHub 长期保存
`,
    hasChat: true,
    hasPlayground: false,
    type: 'lesson' as const,
...
故意给 AI 一段有问题的代码（比如删掉一个分号），看它能不能帮你找出来。把报错贴给 AI，观察分析过程。

> 常见误区
> "AI 报错了肯定是它的问题"——可能是你给的信息不够。把完整报错、代码、环境都告诉 AI，它才能准确定位。
...
开一个新对话，把之前做的项目代码贴进去说"请总结这个项目"。然后用总结在新对话里继续开发。

> 常见误区
> "开一个对话一直聊就行"——对话太长 AI 会遗忘。定期总结、保存代码、新对话贴上下文。

### 你学会了
- 长对话定期总结，新对话贴上下文
- 项目拆成独立模块（HTML/CSS/JS）
- 保存关键代码，出问题能回滚
`,
    hasChat: true,
    hasPlayground: false,
...
选一个你想做的项目，用学到的所有技巧完成它。规划-描述-迭代-保存。完成后把链接分享出去。

> 常见误区
> "等我学完了再做项目"——别等！学一点做一点。第一个项目会很烂，但那是起点。高手都是从烂项目开始的。

### 你学会了
- 开发流程：目标-草图-描述-迭代-测试-发布
- 先完成再完美，能用的烂作品 > 做不出的好作品
- 分享作品，教别人是最好的学习
`,
    hasChat: true,
    hasPlayground: true,
...
分别打开 Claude 和 DeepSeek，用同一个 Prompt 测试，比较回答有什么不同。选一个最舒服的作为主力工具。

> 常见误区
> "贵的工具更好"——入门阶段免费额度足够了。关键是学会描述需求，不是工具本身。

### 你学会了
- 四大主流工具的特点和适用场景
- 初学者推荐 Claude 或 ChatGPT 网页版
- 所有工具都有免费额度，先试用再决定
`,
    hasChat: true,
    hasPlayground: false,
...
打开你选的 AI 工具，说"你好！我想学习编程，你能帮我吗？"——迈出第一步就成功了一半。

> 常见误区
> "我英语不好注册都看不懂"——注册页面都支持中文。遇到英文用浏览器翻译功能就行。
...
在桌面创建 my-first-website 文件夹，新建 index.html，用记事本打开，粘贴 AI 生成的代码，双击在浏览器中查看。

> 常见误区
> "后缀名改了图标没变"——图标关联可能还是旧的。只要确认文件名是 .html 结尾，双击能用浏览器打开就行。
...
随便打开一个网页，按 F12，看 Console 标签页有没有红色报错。有的话复制一条，发给 AI 问"这是什么意思？"

> 常见误区
> "开发者工具好复杂"——现阶段只需：F12 -> Console -> 红色报错复制给 AI。
...
打开 Playground，用课程里的 Prompt 生成个人主页。把你的真实信息填进去，保存为 index.html。

> 常见误区
> "生成的跟我想的不完全一样"——第一次很少完美。说"标题太大了缩小一些"比说"不好看"有效十倍。
...
生成待办清单，添加 5 个任务，勾选 2 个完成，删除 1 个。检查所有功能是否正常。

> 常见误区
> "越做越复杂停不下来"——先做基础功能确认能用，再加功能。每次只加一个。
...
生成一个倒计时工具，设置一个你期待的日子（生日、假期），把它设成浏览器主页。

> 常见误区
> "倒计时不准"——浏览器有 1-2 秒误差是正常的，入门阶段不需要纠结。
...
把前三个项目组合成一个作品集页面。每个项目一个卡片，包含截图和描述。

> 常见误区
> "项目太简陋不好意思展示"——每个项目都是成长记录。三个月后回头看会发现进步巨大。
...
打开浏览器随便访问一个网站。看地址栏：http 还是 https？有没有 www？感受一下域名和协议。

> 常见误区
> "托管和域名要花钱"——GitHub Pages 和 Vercel 免费。域名可以先用免费的 github.io 子域名。

### 你学会了
- 托管 = 文件放服务器上
- 域名 = 服务器网址
- GitHub Pages 是最简单的免费方案
`,
    hasChat: true,
    hasPlayground: false,
    type: 'lesson' as const,
...
注册 GitHub，创建 用户名.github.io 仓库，上传 index.html。1 分钟后访问那个网址——你的网站上线了！

> 常见误区
> "上传了但打不开"——等 1-2 分钟让 GitHub Pages 部署。还不行检查仓库名是否完全正确（必须 用户名.github.io）。

### 你学会了
- GitHub Pages 完全免费
- 仓库名必须是 用户名.github.io
- 上传 index.html 后 1-2 分钟就能访问
`,
    hasChat: true,
    hasPlayground: false,
    type: 'lesson' as const,
